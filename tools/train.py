...
class loss_pseudo_score(nn.Module):
    '''
    Code source: https://github.com/lmy98129/VLPD/blob/main/lib/loss.py
    '''
    def __init__(self):
        super().__init__()
...
def main(args):
    ...
    for epoch in range(start_epoch, args.TRAIN.epochs):
        ...
        if best_mae >= val_mae:
            ...
            filename_model = os.path.join(model_save_dir,'_'+'best.pth')
...
def train_model(
    ...
    contrastive_loss = ContrastiveLoss(temperature=0.07,noise_text_ratio=0., normalize=False)
    pseudo_score_loss = loss_pseudo_score().cuda()
    temp = 2
    qry_loss = 0
    qry_mae = 0
    qry_rmse = 0
    ...
    for i, (query_img, query_den, tokenized_text, class_chosen) in enumerate(train_loader):
        ...
        out, vis_emb, text_emb, _, _ = model(query_img, tokenized_text)

        main = mse_criterion(out, query_den)
        contrast_loss = contrastive_loss(vis_emb, text_emb,  query_den.squeeze(dim=1).detach().clone())
        
        loss = main + 0.001 * contrast_loss
