...
def save_density_map_carpk(query_img, pred_D,GT_D,cnt_err,output_dir, fname='results.png', class_chosen=None, pred_cnt=None):

    if query_img is not None:
        _,h,w = query_img.shape
        query_img = query_img.cpu().numpy()
        # np.save(os.path.join(output_dir,'{}.npy'.format(fname+'_input')), query_img)
    ...
    if pred_D is not None:
        ...
        np.save(os.path.join(output_dir,'{}.npy'.format(fname+'_pred_den')), pred_D)
        if pred_cnt is None:
        ...
        pred_D = .5 * query_img + (1 - .5) * pred_D

    if GT_D is not None:
        GT_D = GT_D.cpu().detach().numpy()
        np.save(os.path.join(output_dir,'{}.npy'.format(fname+'_gt_den')), GT_D)
        gt_cnt = round(np.sum(GT_D))
    ...
    query_result = np.hstack((query_img,pred_D,GT_D))

    cv2.imwrite(os.path.join(output_dir,'{}.jpg'.format(fname)), query_result)
    cv2.imwrite(os.path.join(output_dir,'{}.jpg'.format(fname+'_input')), query_img)
...
def save_density_map(query_img, pred_D,attn, GT_D,output_dir, fname='results.png', class_chosen=None, pred_cnt=None):
    ...
    if pred_D is not None:
        pred_D = pred_D.cpu().detach().numpy()
        np.save(os.path.join(output_dir,'{}.npy'.format(fname)), pred_D)
        if pred_cnt is None:
            pred_cnt = round(np.sum(pred_D), 2)
        pred_D = 255.0 * (pred_D - np.min(pred_D) + 1e-10) / (1e-10 + np.max(pred_D) - np.min(pred_D))
        pred_D = pred_D.squeeze()
        pred_D = cv2.applyColorMap(pred_D[:, :, np.newaxis].astype(np.uint8).repeat(3, axis=2), cv2.COLORMAP_JET)
