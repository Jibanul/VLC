...
def validate_model(
    ...
    with torch.no_grad():
        ...
        text = [template.format("cars") for template in single_plural_prompt_templates]

        from .dataset.tokenizer import tokenize
        ...
        for idx, sample in enumerate(val_loader):
            ...
            save_density_map_carpk(query_img[0],density_map,gt_density,cnt_err,visualize_path,str(idx)+'_cars'+'_gt'+str(round(gt_cnt))+'_pred'+str(round(pred_cnt, 2)),class_chosen="cars", pred_cnt=round(pred_cnt, 2))    #J:
