...
from .models.VLC import Counter 
...
def main(args):
    ...
    # test_loader = get_val_loader(args,mode='val')
    # ====== Test  ======
...
def validate_model(
    ...
    with torch.no_grad():
        for i, (query_img, query_den, tokenized_text, class_chosen, file_name) in enumerate(val_loader):
            ...
            with torch.no_grad():
                while start + 383 < w:
                    output, _, _, _, _ = model(query_img[:, :, :, start:start + 384], tokenized_text)
            ...
            visualize_path = model_save_dir + "/visualize_val_count"
            os.makedirs(visualize_path,exist_ok=True)
            # save_density_map(query_img[0],density_map,density_map,query_den[0],visualize_path,str(epoch)+'_'+class_chosen[0]+'_'+str(i)+'_gt'+str(round(gt_cnt))+'_pred'+str(round(pred_cnt, 2)),class_chosen=class_chosen[0])
            save_density_map(query_img[0],density_map,density_map,query_den[0],visualize_path,str(file_name[0])+'_'+class_chosen[0].replace(" ", "")+'_gt'+str(round(gt_cnt))+'_pred'+str(round(pred_cnt, 2)),class_chosen=class_chosen[0])
