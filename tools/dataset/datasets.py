...
class listDataset(Dataset):
    def __init__(self, data_root, prompt, singular, mode, main_transform):
        ...
        # Load training image list from JSON file
        # We experimented with augmented prompts using ChatGPT, which didn’t result in any improvement.
        if self.prompt == 'chatgpt':
            fsc_chatgpt_path = os.path.join(self.data_root, "fsc_train_chatgpt.json")
            with open(fsc_chatgpt_path) as f:
                self.fsc_train_chatgpt = json.load(f)
    ...
    def __getitem__(self, index):
        ...
        list_path = [self.data_root, 'images_384_VarV2', img_id]
        ...
        image.load()
        gt_path = query_img_path.replace('.jpg','.npy').replace('images_384_VarV2','gt_density_map_adaptive_384_VarV2')
        ...
        elif self.prompt == "chatgpt":
            if self.mode == 'train':
                text = random.sample(self.fsc_train_chatgpt[class_chosen], 11)
            else:
                text = [template.format(engine.plural(class_chosen)) if engine.singular_noun(class_chosen) == False else template.format(class_chosen) for template in self.multi_plural_prompt_templates]
        else:
        ...
        return query_img, query_density, tokenized_text, class_chosen, file_name