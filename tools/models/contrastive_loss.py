...
class ContrastiveLoss(nn.Module):
    ...
    def forward(self, patch_embedding, gt_text_embedding_map, gt_density):
        """
        Args:
            patch_embedding: (B, 576, 512) embedding of image patch feature
        ...
        """
        gt_density = F.interpolate(gt_density.unsqueeze_(1), size=(384, 384), mode='nearest')
        density_mask = F.max_pool2d(gt_density, kernel_size=16, stride=16, padding=0) #same as ViT conv1 
        density_mask = density_mask > 0.
        density_mask = density_mask.permute(0, 2, 3 ,1) # (B, 24, 24, 1)
        gt_text_embedding_map = gt_text_embedding_map.unsqueeze(1).expand(-1, 24, 24, -1) 

        # [B, 24, 24, 512], contains both gt and noise text embedding
        fused_text_embedding_map =  gt_text_embedding_map
        pos_mask = density_mask.squeeze_(-1) # (B, 24, 24, 1)
        
        patch_embeddings = patch_embedding.reshape(-1, 24, 24, 512)
        
        #batch cosine similarity, this function automatically normalizes the vectors
        sim_map = F.cosine_similarity(patch_embeddings, fused_text_embedding_map , dim=-1) # (B, 24, 24)
